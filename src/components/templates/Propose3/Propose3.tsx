...
export default function Propose3({
  ...
  useEffect(() => {
    ...
    const runPreload = async () => {
      ...
      const assets = [
        customData.s1_img, customData.s2_img, customData.s3_img, customData.s4_img,
        customData.photo1, customData.photo2, customData.photo3, customData.photo4, customData.photo5,
    ...
    <div style={{
      ...
      {preloading && (
        <div style={{ position: "fixed", inset: 0, zIndex: 99999, background: "#06060A", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", fontFamily: "'Inter', sans-serif" }}>
          <div style={{ width: 44, height: 44, borderRadius: "50%", border: "3.5px solid rgba(236,72,153,0.15)", borderTopColor: "#EC4899", animation: "tmpl-spin 0.9s cubic-bezier(0.16, 1, 0.3, 1) infinite", marginBottom: 20 }} />
          <h2 style={{ fontWeight: 800, color: "#FFFFFF", fontSize: 18, letterSpacing: -0.3, animation: "tmpl-pulse 2s infinite", marginBottom: 16 }}>Opening Your Surprise</h2>
          
          <div style={{ width: 240, height: 4, background: "rgba(255,255,255,0.06)", borderRadius: 2, overflow: "hidden", marginBottom: 12 }}>
            <div style={{ height: "100%", background: "#EC4899", width: `${preloadProgress}%`, transition: "width 0.3s ease" }} />
          </div>
          <p style={{ color: "#EC4899", fontSize: 13, opacity: 0.8 }}>{preloadText}</p>
